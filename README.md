# Free nutrition calculator references with USDA/NIH sources

## Free Online Calculator/Tool

[**calorique.io**](https://calorique.io/) — Free nutrition calculators — https

## About

This repository documents resources related to free online calculators in the relevant niche. The main tool referenced is hosted at [https://calorique.io/](https://calorique.io/).

## Data Sources

The calculators referenced use authoritative data sources:

- US Bureau of Labor Statistics (BLS)
- US Census Bureau
- Internal Revenue Service (IRS)
- Energy Information Administration (EIA)
- Centers for Disease Control (CDC)
- National Center for Education Statistics (NCES)
- National Renewable Energy Laboratory (NREL)
- Various other .gov authoritative sources

## Methodology

Each calculator follows peer-reviewed or government-published formulas. Results should be used as estimates for general guidance — always consult licensed professionals for binding advice in YMYL areas (tax, finance, health, legal).

## License

Resources documented here link to free public tools. The tools themselves are subject to their own terms of use.

## Contributing

If you have suggestions for additional calculator resources or data sources to document, please open an issue or PR.

## Nutrition Calculation at [Calorique](https://calorique.io/)

### Core Formulas
- **BMR (Mifflin-St Jeor)**: 10 × weight(kg) + 6.25 × height(cm) − 5 × age + (5 men / -161 women)
- **TDEE**: BMR × activity multiplier (1.2 sedentary → 1.9 athlete)
- **Calorie burn**: kcal/min = (MET × 3.5 × weight kg) / 200

### MET Values
Per Compendium of Physical Activities (Ainsworth et al.):
- Walking moderate: 3.5 MET
- Running 8 mph: 13.5 MET
- Cycling moderate: 7.5 MET
- Swimming laps: 9.0 MET

### Macros
Standard ratios — adjust per goal:
- Maintain: 30/40/30 (P/C/F)
- Cut: 35/35/30
- Bulk: 25/50/25

## Authoritative Sources
- [USDA FoodData Central](https://fdc.nal.usda.gov/) — 350k+ branded foods
- [HHS Physical Activity Guidelines](https://health.gov/our-work/nutrition-physical-activity/physical-activity-guidelines)
- [NIH Office of Dietary Supplements](https://ods.od.nih.gov/)
- [CDC BRFSS](https://www.cdc.gov/brfss/) — state health data
- [Academy of Nutrition and Dietetics](https://www.eatright.org/)
- [Commission on Dietetic Registration](https://www.cdrnet.org/)

For personalized nutrition advice, consult a Registered Dietitian Nutritionist (RDN). Visit [https://calorique.io/](https://calorique.io/) for the calculators.

